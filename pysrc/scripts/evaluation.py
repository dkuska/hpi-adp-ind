...
from ..utils.plots import (create_tp_fp_fn_stacked_barplot_by_method, create_plot, plot_missing_values)
    ...
    with open(output_csv, 'w') as csv_output:
        ...
        if runs.baseline.configuration.arity == 'unary':
            writer.writerow(['file_names', 'sampling_method', "budgets", 'tp', 'fp', 'fn', 'precision', 'recall', 'f1',
                             'mean_tp_missing_values', 'mean_fp_missing_values'])
        else:
            writer.writerow(['file_names', 'sampling_method', "budgets", 'tp', 'fp', 'fn', 'precision', 'recall', 'f1'])
...
def plotting_preprocessing_evaluation_dataframe(df: pd.DataFrame,
                                                arity: str) -> tuple[pd.DataFrame, Optional[pd.DataFrame]]:
    """After loading the data from the evaluation csv,
    some preprocessing needs to be done, before we can create plots"""
    ...
    if arity == 'nary':
        ...
        def get_map(*, dtype: type, avg: bool = False) -> Callable[[Any], Any]:
            return lambda x: sum([dtype(i) for i in x]) / (len(x) if avg else 1)
        ...
        df_unary = df.assign(tp=get_assign('tp'),
                             ...
                             f1=get_assign('f1'))
        df['tp'] = df['tp'].map(get_map(dtype=int))
        df['fp'] = df['fp'].map(get_map(dtype=int))
        df['fn'] = df['fn'].map(get_map(dtype=int))
        df['precision'] = df['precision'].map(get_map(dtype=float, avg=True))
        df['recall'] = df['recall'].map(get_map(dtype=float, avg=True))
        df['f1'] = df['f1'].map(get_map(dtype=float, avg=True))
...
def make_plots(output_file: str) -> list[str]:
    ...
    for sampling_method, _ in df_original.groupby('sampling_method'):
        # TODO: Is this assumption safe? When would it break?
        if not isinstance(sampling_method, str):
            raise TypeError(f'{df_original=} produced incorrect sampling method of type {type(sampling_method)} (value: {sampling_method}).')
        sampling_methods.append(sampling_method)

    plot_fname = f'{plot_prefix}_stackedBarPlots_detailed.jpg'
    plot_path = create_plot(df_original, sampling_methods, create_tp_fp_fn_stacked_barplot_by_method,
                            plot_prefix, plot_fname)
...
def collect_ind_ranking(experiments: MetanomeRunBatch, mode: Literal['interactive', 'file'],
                        output_folder: str, top_inds: int) -> str:
    ranked_inds = experiments.ranked_inds()
    baseline = experiments.baseline
    ranked_inds_object = [RankedIND(ind, credibility, baseline.results.has_ind(ind))
                          for ind, credibility
                          in ranked_inds.items()]
    if mode == 'interactive':
...
def evaluate_ind_rankings(ranked_inds_path: str, maximum_threshold_percentage: float) -> None:
    ...
    for ranked_ind in ranked_inds:
        if maximum_threshold_percentage > 0.0 and ranked_ind.credibility >= maximum_threshold_percentage * max_credibility \
            ...
            # When threshold == 0, still require results to be > 0 for better results
...
def run_evaluation(config: GlobalConfiguration, file: str, interactive: bool,
                   return_path: str, top_inds: int) -> Optional[str]:
    ...
    if config.create_plots:
        ...
        plot_paths = []
    ranked_inds_path = collect_ind_ranking(experiments, 'interactive' if interactive else 'file',
                                           output_sub_directory, top_inds)
    if interactive:
        thresholds = [1.0, 0.995, 0.99, 0.95, 0.9, 0.8, 0.7, 0.6, 0.5, 0.45, 0.4,
                      0.35, 0.3, 0.25, 0.2, 0.15, 0.1, 0.01, 0.005, 0.001, 0.0]
...
def run_evaluations(general_config: GlobalConfiguration,
                    evaluation_config: EvaluationConfiguration) -> list[Optional[str]]:
    if evaluation_config.file is not None:
        return [run_evaluation(general_config, evaluation_config.file, evaluation_config.interactive,
                               evaluation_config.return_path, evaluation_config.top_inds)]
    return [run_evaluation(general_config, file.rstrip(), evaluation_config.interactive,
                           evaluation_config.return_path, evaluation_config.top_inds)
            for file
            in sys.stdin.read().split('\0')]
...
def main() -> None:
    ...
    if not general_config.pipe and evaluation_config.file is None \
            or general_config.pipe and evaluation_config.file is not None:
