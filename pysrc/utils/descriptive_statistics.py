...
def file_column_statistics(file_path: str, header: bool, *, delimiter: str =';', escapechar: str ='\\', is_baseline: bool = False) -> list[ColumnStatistic]:
    ...
    if is_non_zero_file(file_path):
        df = pd.read_csv(file_path, delimiter=delimiter, escapechar=escapechar, dtype=str, on_bad_lines='skip', header='infer' if header else None) 
        ...
        descriptions.append(ColumnStatistic(column_information=ColumnInformation(table_name=table_name, column_name=column_name),
                                    count=0,
                                    unique_count=0,
                                    unique_ratio=0,
                                    min='',
